...
train_pipeline = [
    ...
    dict(type='RandomResize', scale=(1024, 1024), ratio_range=(0.1, 2.0), keep_ratio=True),
    dict(type='RandomCrop', crop_type='absolute_range', crop_size=(1024, 1024), allow_negative_crop=True, recompute_bbox=True),
...
model = dict(
    ...
    data_preprocessor=dict(
        ...
        bgr_to_rgb=True,
        pad_mask=True,
        batch_augments=[dict(type='BatchFixedSizePad', size=(1024, 1024), pad_mask=True)]
    ),
    backbone=dict(
...
val_evaluator = dict(
    type='CocoMetric',
    ann_file=data_root + 'val.json',  # 'recyclingval.json' 대신 'val.json'으로 수정 (실제 파일명 확인 필요)
    metric='bbox',
    backend_args=None
)
